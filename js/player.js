 ...
 */ (function ($, window, document, undefined) {
    $.fn.player = function (method) {
        var pluginName = 'jquery-youtube-player',
            args = arguments,
            val = undefined;
        this.each(function () {
    ...
    function player(element, options, pluginName) {
        ...
        if (this.element.is('a')) {
            ...
            var playerVideo = $('<div class="youtube-player-video"></div>').appendTo(this.element),
                playerObject = $('<div class="youtube-player-object"></div>').appendTo(playerVideo),
                playlist = $('<ol class="youtube-player-playlist"><li></li></ol>').find('li').append(anchor.clone()).end().appendTo(this.element);
        ...
        (this.options.swfobject.getFlashPlayerVersion().major >= 8) && this.loadPlaylist(null, null, null, function () {
            // build everything and set event handlers
            self._bindYoutubeEventHandlers()._createToolbar()._createTimeArea()._createPlayer();
        });
    }
    // public members
    player.prototype = {
        ...
        _bindYoutubeEventHandlers: function () {
            ...
            function ready(id) {
                ...
                self.elements.toolbar.container.animate(self.options.toolbarAnimation, self.options.toolbarSpeed, function () {
        ...
        _createPlayer: function () {
            ...
            var id = this.options.playlist.videos[this.keys.video].id,
                ...
                swfpath = this.options.chromeless ? 'http://www.youtube.com/apiplayer?enablejsapi=1&version=3&playerapiid=' + apiid + '&hd=' + this.options.highDef + '&showinfo=0' : 'http://www.youtube.com/v/' + id + '?enablejsapi=1&playerapiid=' + apiid + '&version=3';
        ...
        _createToolbar: function () {
            ...
            this.elements.toolbar = {
                container: $('<ul />').addClass('youtube-player-toolbar ui-widget ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all').css('opacity', 0),
                updateStates: function () {
                    self.elements.toolbar.container.find('li').each(function () {
                        var button = $(this).removeClass('ui-state-active').data('button');
                        if (!button) return true;
                        (self._state(button.val)) && (button.toggle) && $(this).addClass('ui-state-active');
            ...
            $.each(this.options.toolbar.split(','), function (key, val) {
                ...
                $('<li></li>').addClass('ui-state-default ui-corner-all').append('<span class="ui-icon ' + button.icon + '"></span>').attr('title', button.text).data('button', button).bind('mouseenter mouseleave', function () {
                    $(this).toggleClass('ui-state-hover');
                }).bind('off', function () {
                    var elem = $(this),
                        button = elem.data('button'),
                        toggle = 1;
                    elem.data('toggle', toggle);
                    self._removeState(val);
                    elem.find('.ui-icon').removeClass(button.toggleButton.icon).addClass(button.icon).end().attr('title', button.text);
                    ...
                    elem.data('toggle', toggle);
                    self._addState(val);
                    elem.find('.ui-icon').removeClass(button.icon).addClass(button.toggleButton.icon).end().attr('title', button.toggleButton.text)
                    ...
                    if (button.toggleButton) {
                        ...
                        self._trigger(self, button.action, [button]);
                        (!button.toggle || (button.toggle && state)) ? self._removeState(val) : self._addState(val);
                        self.elements.toolbar.updateStates();
                    }
                }).appendTo(self.elements.toolbar.container);
            });
            (this.options.toolbarAppendTo) ? this.elements.toolbar.container.appendTo(this.options.toolbarAppendTo) : this.elements.playerVideo.after(this.elements.toolbar.container);
            return this;
        },
        _createTimeArea: function () {
            this.elements.toolbar.time = this.options.timeAppendTo ? $('<span />').appendTo(this.options.timeAppendTo) : $('<li />').addClass('youtube-player-time').appendTo(this.elements.toolbar.container);
        ...
        _createPlaylist: function () {
            ...
            function buildPlaylist() {
                self.elements.playlist = self.elements.playlist || $('<ol />').addClass('youtube-player-playlist ui-helper-reset');
                self.elements.playlistContainer = self.elements.playlistContainer || $('<div />').addClass('youtube-player-playlist-container ui-widget-content ui-corner-all').html(self.elements.playlist);
            };
            this._addVideosToPlaylist = function (cue) {
                ...
                $.each(videos, function () {
                    ...
                    $('<li />').data('video', this).append(self.options.videoThumbs ? '<img alt="' + this.title + '" title="' + this.title + '" src="http://img.youtube.com/vi/' + this.id + '/2.jpg" />' : this.title).addClass('ui-state-default').addClass(self.options.videoThumbs ? 'youtube-player-thumb' : '').bind('mouseenter mouseleave', function () {
                        $(this).toggleClass('ui-state-hover');
        ...
        _updateTime: function () {
            ...
            var self = this,
                duration = Number(this.youtubePlayer.getDuration());
            ...
            this.timeInterval = setInterval(function () {
                (!self.youtubePlayer.getCurrentTime) ? clearInterval(self.timeInterval) : self.elements.toolbar.timeCurrent.html(timeFormat(self.youtubePlayer.getCurrentTime()));
            }, 100);
        },
        ...
        _addState: function (state, stateID) {
            if (stateID) {
                $.inArray(state, this._activeStates) === -1 && this._activeStates.push(state);
            } else {
                this._states[state] && $.inArray(this._states[state], this._activeStates) === -1 && this._activeStates.push(this._states[state]);
            }
        },
        ...
        _getPlaylistData: function (success, error) {
            ...
            if (playlist.user || playlist.playlist) {
                ...
                var url = playlist.user ? 'http://gdata.youtube.com/feeds/api/videos' : 'http://gdata.youtube.com/feeds/api/playlists/' + playlist.playlist;
                url += '?callback=?';
                var data = {
                    alt: 'json',
                    format: '5'
        ...
        _updatePlaylist: function () {
            var self = this;
            (this.elements.playlist) && this.elements.playlist.find('li').removeClass('ui-state-active').each(function (key) {
        ...
        _showPlaylist: function (callback) {
            ...
            var oldHeight = this.elements.playlist.height(),
        ...
        loadVideo: function (video, cue) {
            ...
            function load(videoID) {
                (cue) ? self.cueVideo(videoID) : self.youtubePlayer.loadVideoById(videoID, 0);
        ...
        playlistToggle: function (button) {
            (this.elements.playlistContainer.find('li').length) && this.elements.playlistContainer.animate({
                height: 'toggle',
                opacity: 'toggle'
